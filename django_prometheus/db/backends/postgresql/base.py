...
class DatabaseWrapper(DatabaseWrapperMixin, base.DatabaseWrapper):
    def get_connection_params(self):
        ...
        conn_params['cursor_factory'] = ExportingCursorWrapper(psycopg2.extensions.cursor,
                                                               self.alias,
                                                               self.vendor)
        return conn_params

    def create_cursor(self):
        # cursor_factory is a kwarg to connect() so restore create_cursor()'s default behavior
